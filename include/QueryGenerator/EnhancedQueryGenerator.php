			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($baseFieldName, $this->referenceFieldList)) {
					...
					if ($parentReferenceField)
						...
						$ownerTableName = '';
					$concatSql = getSqlForNameInDisplayFormat(array('last_name' => 'vtiger_users'.$ownerTableName.'.last_name',
						'first_name' => 'vtiger_users'.$ownerTableName.'.first_name'), 'Users');
	...
	function getOrderByColumn($fieldName) {
		...
		if ($orderByFieldModel && $orderByFieldModel->getFieldDataType() == 'reference') {
			...
			if (in_array('DocumentFolders', $referenceModules)) {
				...
				$columnSqlTable = 'vtiger_users'.$parentReferenceField.$fieldName;
				$orderByColumn = getSqlForNameInDisplayFormat(array('last_name' => $columnSqlTable.'.last_name',
			...
			if ($parentReferenceField) {
				...
				$orderByColumn = "COALESCE(CONCAT($userTableName.last_name,$userTableName.first_name),$groupTableName.groupname)";
			} else {
